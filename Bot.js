...
class Bot {
  ...
  docsGetWallUploadServer (group_id, callback, attempt) {
    ...
    self.api('docs.getWallUploadServer', {
      group_id: group_id,
      type: 'audio_message'
    }, function (data) {
        ...
        for (var i = errorsMethods.length - 1; i >= 0; i--) {
          try {
            ...
            for (var n = keys.length - 1; n >= 0; n--) {
              error.request_params.push({
                key: keys[n],
                value: params[keys[n]]
              })
            }
            self.CallbackRegistry[errorsMethods[i]]({
              error: error
            })
          } catch (ignored) {}
        }
  ...
  api (method, params, callback, attempt) {
    ...
    if (method != 'execute') {
      ...
      self.methodQueue.push({
        callbackName: callbackName,
        method: 'API.' + method + '(' + JSON.stringify(params) + ')',
        params: params
      })
      return
    }
